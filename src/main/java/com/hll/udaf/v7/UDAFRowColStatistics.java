...
public class UDAFRowColStatistics extends UDAF {

    public static Logger logger = Logger.getLogger(UDAFRowColStatistics.class);
    ...
    public static class Evaluator implements UDAFEvaluator {
        ...
        public boolean iterate(List<String> input0, List<String> compare, List<String> measure_value,
            ...
            String dimension_key = dimensions.stream().reduce((a, b) -> String.format("%s\001%s", a, b)).get();
            dimensions.addAll(compare);
            String compare_key = dimensions.stream().reduce((a, b) -> String.format("%s\001%s", a, b)).get();
            // compare_length
            ...
            for (Map.Entry<String, String> mea : measure.entrySet()) {
                ...
                String mea_key = compare_key + "\001" + key;
            ...
            for (Map.Entry<String, String> en : measure.entrySet()) {
                ...
                String dimension_tmp_key = String.format("%s\001%s", dimension_key, key);
        ...
        public Map<String, Double> terminate() {
            ...
            for (Map<String, String> value : cat.values()) {
                for (String measure_key : value.keySet()) {
                    ...
                    String compare_key = measure_key.substring(0, measure_key.lastIndexOf("\001"));
                    String compare_key_tmp = measure_key.substring(0, measure_key.lastIndexOf("\001"));
                    String[] all_keys = compare_key.split("\001");

                    // create dimension key
                    String dimensionKeys = all_keys[0];
                    for (int i = 1; i < dimension_length; i++) {
                        try {
                            dimensionKeys = String.format("%s\001%s", dimensionKeys, all_keys[i]);
                        } catch (Exception e) {
                            dimensionKeys = String.format("%s\001%s", dimensionKeys, "");
                        }
                    ...
                    if (compare_length > 0) {
                        ...
                        for (int i = (dimension_length + 1); i < (dimension_length + compare_length); i++) {
                            compareKeys = compareKeys + "\001" + all_keys[i];
                        }
                    }
                    ...
                    for (int i = 0; i < measure_arr.length; i++) {
                        ...
                        String tmp_key = String.format("%s\001%s", compare_key, s);
                        ...
                        double submeasure = subMeasureSum.getOrDefault(dimensionKeys + "\001" + s, 0.00);
                        ...
                        if (row_func[i + 1].equals("sum-1") || row_func[i + 1].equals("avg-1")) {
                            ...
                            subMeasureSum.put(String.format("%s\001%s", dimensionKeys, s), submeasure);
                        ...
                        compare_key_tmp = String.format("%s\001%s", compare_key_tmp, v);
                    }

                    // 行合计
                    for (int i = 0; i < measure_arr.length; i++) {
                        String s = measure_arr[i];
                        double v = dimension.getOrDefault(dimensionKeys + "\001" + s, 0.00);
            ...
            for (Map.Entry<String, Double> en : result.entrySet()) {
                ...
                for (int i = 0; i < measure_arr.length; i++) {
                    double v = subMeasureSum.getOrDefault(dkey + "\001" + measure_arr[i], 0.00);
                ...
                if (Integer.parseInt(rowcol) == 1 || Integer.parseInt(rowcol) == 7) {
                    r = r + "\001" + rowsum + "\001" + measure_total + "\001";
            ...
            if (rowcol.equals("7")) {
                ...
                for (String k : result_bak.keySet()) {
                    ...
                    List<String> measure_list = splitArray(k.split("\001"), dimension_length + compare_length, dimension_length + compare_length + measure_length);

                    String total_pre_key = total_key;
                    if (compare_list != null && compare_list.size() > 0) {
                        total_pre_key = String.format("%s\001%s", total_key, compare_list.stream().reduce((a, b) -> String.format("%s\001%s", a, b)).get());
                    }
                ...
                for (Map.Entry<String, Integer> en : total_keyCount.entrySet()) {
                    String key = en.getKey();
                    int i = en.getValue();
                    String value = totalSumMap.getOrDefault(key, "0\0010");
...
//                        totalSumMap.put(key, vrr[0] + "\001" + vrr[1]);
                        StringBuilder sb =new StringBuilder();
                        for (int j = 0; j < vrr.length; j++) {
                            sb.append(vrr[j]).append("\001");
                        }
                ...
                for (Map.Entry<String, String> en : totalSumMap.entrySet()) {
                    ...
                    String[] measure_value = getKey(value, measure_length).split("\001");
                    ...
                    for (int j = 0; j < measure_arr.length; j++) {
                        ...
                        String submeasureKey = dkey + "\001" + measure_arr[j];
                ...
                for (Map.Entry<String, Integer> en : subtotal_tmp_keyCount.entrySet()) {
                    String key = en.getKey();
                    int i = en.getValue();
                    String value = sub_ttal.getOrDefault(key, "0\0010");
                    String[] vrr = value.split("\001");
                    for (int j = 0; j < vrr.length; j++) {
...
//                        sub_ttal.put(key, vrr[0] + "\001" + vrr[1]);
                        StringBuilder sb =new StringBuilder();
                        for (int j = 0; j < vrr.length; j++) {
                            sb.append(vrr[j]).append("\001");
                        }
                ...
                for (Map.Entry<String, String> en : sub_ttal.entrySet()) {
                    ...
                    String[] valueArr = value.split("\001");
                ...
                for (Map.Entry<String, String> i : totalSumMap.entrySet()) {
                    ...
                    String submeasure = "\001";
                    for (int j = 0; j < measure_arr.length; j++) {
                        double v = total_numerical_subtotal.getOrDefault(dkey + "\001" + measure_arr[j], 0.00) / size;
                        if (row_func[j + 1].equals("avg-1")) {
                            v = v / (size);
                        } else {
                            v = v;
                        }
                        submeasure = submeasure + (v) + "\001";
                    }
                    ...
                    result_bak.put(i.getKey() + "\001" + measure + "" + submeasure + "" + rowsum + "\001" + measure_total + "\001columnSum", 0.00);
                }
                ...
                for (Map.Entry<String, String> i : sub_ttal.entrySet()) {
                    ...
                    for (int j = 0; j < measure_arr.length; j++) {
                        double v = sub_numerical_subtotal.getOrDefault(dkey + "\001" + measure_arr[j], 0.00) / size;
                        if (row_func[j + 1].equals("avg-1")) {
                            v = v / (size);
                        } else {
                            v = v;
...
//                    int size = sub_dimenkey2compareSize7.getOrDefault(dkey, 1);
                    ...
                    result_bak.put(i.getKey() + "\001" + measure + "" + submeasure + "" + rowsum + "\001" + measure_total + "\001columnSum_subtotal_", 0.00);
                }
                ...
                for (String k : result_bak.keySet()) {
                    ...
                    if (compare_list != null && compare_list.size() > 0) {
                        total_tmp_key = total_tmp_key + "\001" + compare_list.stream().reduce((a, b) -> a + "\001" + b).get();
                    }
                    Integer y = total_keyCount.getOrDefault(total_tmp_key, 0);
                    String[] tt = totalSumMap.getOrDefault(total_tmp_key, mea.toString()).split("\001");
                    StringBuilder res = new StringBuilder();
                    for (int j = 0; j < measure_list.size(); j++) {
                        double o = Double.parseDouble(tt[j]);
                        double p = Double.parseDouble(measure_list.get(j));
                ...
                for (String k : result_bak.keySet()) {
                    ...
                    for (int i = 1; i < dimension_list.size(); i++) {
                        ...
                        String total_tmp_key = total_key;
                        if (compare_list != null && compare_list.size() > 0) {
                            subtotal_tmp_key = subtotal_tmp_key + "\001" + compare_list.stream().reduce((a, b) -> a + "\001" + b).get();
                        }
                        Integer count = subtotal_tmp_keyCount.getOrDefault(subtotal_tmp_key, 0);
                        String[] tt = sub_ttal.getOrDefault(subtotal_tmp_key, mea.toString()).split("\001");
                        String func2 = splitArray(col_func, measure_length, measure_length + 1).get(0);
                ...
                if (rowcol.equals("4") && !rowcol.equals("2")) {
                    for (Map.Entry<String, String> en : totalSumMap.entrySet()) {
                        String key = String.format("%s\001%s\001columnSum", en.getKey(), en.getValue());
                        result_bak.put(key, 0.00);
                    }
                }
                if (!rowcol.equals("4") && rowcol.equals("2")) {
                    for (Map.Entry<String, String> en : sub_ttal.entrySet()) {
                        String key = String.format("%s\001%s\001columnSum_subtotal_", en.getKey(), en.getValue());
                        result_bak.put(key, 0.00);
                    }
                }
                if (rowcol.equals("6")) {
